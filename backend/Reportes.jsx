...
function Reportes() {
  ...
  const obtenerPrediccion = async () => {
    ...
    try {
      ...
      // Llamada exacta sin barra final para evitar redirecciones 307
      ...
      console.error("Error crítico de red o IA:", error);

      // Degradación Elegante: Mensaje amigable al usuario
      setProductoIA("Servicio no disponible");
      setRecomendacionIA("Error de conexión (Network Error). El motor de IA podría estar reiniciándose o el servidor está bajo mucha carga.");
      setNivelIA("Desconocido");
    } finally {
      setCargando(false); // Garantiza que el botón se desbloquee siempre
    }
  }
